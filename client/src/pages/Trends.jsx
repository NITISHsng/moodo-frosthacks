...
import { Calendar, Filter, Download, BrainCircuit, HeartPulse, Smile, TrendingDown } from 'lucide-react';
import { MoodChart } from '../components';
import { StatCard } from '../components';
import { InsightCard } from '../components';
import { FluctuationItem } from '../components';
...
const Trends = () => {
  ...
  const stats = [
    { label: 'Weekly Average', value: avgMood.toFixed(1), icon: BrainCircuit, color: 'text-primary', bg: 'bg-primary/10' },
    { label: 'Peak Mood', value: peakMood.toFixed(1), icon: Smile, color: 'text-emerald-600', bg: 'bg-emerald-50' },
    { label: 'Lowest Mood', value: lowestMood.toFixed(1), icon: TrendingDown, color: 'text-red-600', bg: 'bg-red-50' },
    { label: 'Stability', value: dashboard?.fluctuation < 2 ? 'High' : 'Low', icon: HeartPulse, color: 'text-purple-600', bg: 'bg-purple-50' },
  ];
  ...
  const fluctuations = (dashboard?.alerts || []).map(alert => ({
    day: new Date(alert.createdAt).toLocaleDateString('en-US', { weekday: 'long' }),
  ...
  // AI Insights mapping
  const insightMessages = dashboard?.alerts?.map(alert => alert.message) || [];
  if (insightMessages.length === 0) {
    insightMessages.push(`Your mood average is ${avgMood.toFixed(1)} this week.`);
        ...
        <div className="bg-card rounded-3xl p-8 border border-border shadow-sm space-y-6">
          ...
          <div className="space-y-4">
            {fluctuations.length > 0 ? (
                ...
                <p className="text-muted-foreground text-sm">No significant fluctuations detected recently.</p>
            )}
          </div>
        </div>
      </div>
